...
# generated waveforms. This uses the operating system's 
# built-in more secure source of randomness.
#
...
load_crypto_random_waves() {
  ...
  echo -e "Generating new crypto random waveforms and 
loading them onto the signal generator device, overwriting 
all currently saved user waveforms. This usually takes 
about 18 or 19 minutes...\n"
  ...
  $(\
    ...
    for i in {1..60}; do
      ...
      # Print this message to stdout of the parent shell using 
      # our alias from above.
      echo -e "Generating new crypto random waveform and saving it in slot:\t$i of 60" >&3

      # Upload ASCII waveform data from stdin to the device.
      $COMMAND --wws $i <<< \
        `for j in {0..2047}; do
          # Output crypto random values in the range of 0 - 4095, 
          # 2048 times, one value per line.
          echo $(od -An -N2 -i /dev/urandom | xargs) % 4096 | bc
        done`
